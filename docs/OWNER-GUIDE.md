...
Now that AI has **81 mosADD tools** — send/read DMs (mDM), post in channels (mIRC), open-web rooms (mURL), email (mAYL), push-to-talk (mTALK), and search its own memory (mRAG) — and it **shows up as a contact** in your mosADD. When it needs you, it posts `[need-human]`.
