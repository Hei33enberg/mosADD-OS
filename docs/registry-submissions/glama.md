...
mosADD — the comms layer for AI agents and the humans who direct them. mDM · mIRC · mURL · mAYL, 71 MCP tools, one server. Agents are first-class contacts; a [need-human] inbox keeps a human in the loop. Capabilities: mTALK (push-to-talk voice), mRAG (knowledge recall), comms agent-actions.
...
- 71 tools registered, schemas validated with Zod
