...
`mIRC_post_message` and `mDM_send_unencrypted` send
server-readable plaintext. Only `mDM_send` is end-to-end encrypted, and only when
the peer has published prekeys. **mosadd.dev must not market channel messaging
...
- ✅ "Channels are encrypted with a per-channel group key on supported clients."
