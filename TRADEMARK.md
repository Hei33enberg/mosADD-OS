# Trademark Policy

"mosadd", the radar device used in mosadd marketing, and the `m·os·add` wordmark are unregistered trademarks of the mosadd commercial entity ("mosadd"). The mosadd-os source code is licensed under [Apache-2.0](./LICENSE) and may be used freely, but the trademark grants below are separate from the source-code licence.

## What you may do without asking

- Refer to mosadd by name in documentation, articles, talks, blog posts, social media, and other editorial contexts
- Say your project uses, integrates with, or is compatible with mosadd
- Distribute unmodified copies of the mosadd-os source code under the Apache-2.0 licence
- Run mosadd-os in production for your own users or organisation, with or without modification
- Fork mosadd-os and continue development under Apache-2.0 — including under a different name
- Use the mosadd name to describe what such a fork derives from ("based on mosadd", "originally forked from mosadd-os")

## What requires our permission

- Using "mosadd" or the radar logo as the name of your own product, service, company, or domain
- Distributing modified versions of mosadd-os under the "mosadd" name without an obvious "unofficial" / "community" disclaimer
- Selling t-shirts, stickers, merchandise, or other physical goods bearing the mosadd name or logo
- Using the mosadd name in a way that suggests endorsement or affiliation when none exists

If you want to do any of the above, email `trademark@mosadd.com` and we'll talk. We're friendly — the goal of this policy is to prevent user confusion, not to be a gatekeeper.

## What you must not do

- Register "mosadd" or confusingly-similar names as your own trademark in any jurisdiction
- Use the mosadd name in a way that disparages mosadd contributors or maintainers
- Use the mosadd name to promote products or services that violate mosadd's [Code of Conduct](./CODE_OF_CONDUCT.md)

## Names of forks

If you fork mosadd-os, please pick a name that doesn't include "mosadd" as a distinct word. For example:

- ✅ `comm-fork-by-acme` — fine
- ✅ `mosadd-community` — fine (you're clearly community-flavoured)
- ⚠️ `MosaddPro`, `mosadd-enterprise`, `mosadd-cloud` — suggests official affiliation; please reach out first
- ❌ `mosadd` (any other org / npm scope) — please don't

## Why this matters

mosadd's commercial offering (the hosted hub at `hub.mosadd.com` and the managed MCP gateway at `mcp.mosadd.com`) is what funds open-source maintenance of mosadd-os. Brand confusion that pulls people from "official mosadd" to "third-party-mosadd-pretending-to-be-official" damages that funding loop and the project's sustainability. Hence this policy.

The source code remains free forever under Apache-2.0. The brand stays guarded so we can keep the source code free forever.

## Questions

Email `trademark@mosadd.com` for trademark questions. For general questions, use [GitHub Discussions](https://github.com/Hei33enberg/mosadd-os/discussions) or our Discord (link in [README.md](./README.md)).
