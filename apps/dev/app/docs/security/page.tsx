  ...
  return (
    <Prose>
      ...
      <Ul>
        ...
        <li><strong>Irondome — on-device threat pillar.</strong> Flags device-integrity and network anomalies (rooted / tampered devices, sideloads, MITM) and mercenary-spyware <em>indicators</em> — including DNS lookups of known C2 domains — shipping as an embeddable package (<code className="font-mono text-primary">@mosadd/threat-engine</code>). Detection runs on your device; the <em>engine</em> is a pure function with no network, but mLIDAR&apos;s findings are recorded to your account, so it is not a purely local tool. Opt-in, off by default, and signal-only — it flags indicators and never acts, and it is not a forensic off-device scan. See <Anchor href="/docs/modules/threat-engine">threat-engine</Anchor>.</li>
