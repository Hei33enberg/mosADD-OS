...
async function fetchActivity(slug: string): Promise<{ messages: number; status: string } | null> {
  try {
    const r = await fetch(`https://rooffhgbxafyjcwmwpsy.supabase.co/functions/v1/channel0-trending?minutes=60`, {
      cache: 'no-store',
    });
    if (!r.ok) return null;
