...
// Full-site password gateway for murl.mosadd.com.
// Parked alongside mosadd.dev. Set `GATE_PASSWORD` on Vercel to override.
const PASSWORD = process.env.GATE_PASSWORD ?? 'mosadd2026';
const COOKIE = 'mosadd_murl_gate';
const UNLOCKED = 'ok';
const MAX_AGE = 60 * 60 * 24 * 30;

export const config = {
  matcher: [
    '/((?!_next/static|_next/image|_next/data|favicon\\.ico|robots\\.txt|sitemap\\.xml|opengraph-image|twitter-image|apple-icon|icon|manifest\\.webmanifest).*)',
  ],
};
...
export default async function middleware(req: NextRequest) {
  ...
  // request, then the cookie does the work forever.
