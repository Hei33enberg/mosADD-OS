...
Serves the full `@mosadd/mcp` tool surface (the 6 live modules, 52 tools) over
...
Verified locally: `initialize` → 200, `tools/list` → 52 tools (mDM/mIRC/mROOM/
mTALK/mAIL/mRAG), `tools/call mROOM_list` → real backend response, bad key → 401.
